...
/* 
 * Native Liquid-Glass stylized Share Modal
 * Features an Apple-inspired native spring animation
...
export const ShareModal: React.FC<ShareModalProps> = ({
  ...
  // Adaptive background for the sheet content over the glass layer
  const sheetContentBg = isIOS 
    ? (isDark ? 'rgba(25, 25, 28, 0.7)' : 'rgba(248, 248, 250, 0.8)')
    : colors.card; 
  ...
  return (
    <Modal visible={showModal} animationType="none" transparent={true} onRequestClose={handleClose}>
      ...
      <View style={styles.sheetLayoutContainer} pointerEvents="box-none">
        {/* We use touchable without feedback here just in case they click exactly outside the sheet but in the lower bounds */}
        <Animated.View style={[
            styles.sheetWrapper, 
            { 
        ...
        >
          ...
          <GlassCard cornerRadius={36} intensity={isDark ? 30 : 60} style={styles.glassContainer}>
            <View style={[
                styles.sheetContent, 
                { 
              ...
              <View style={styles.header}>
                ...
                <TouchableOpacity style={[styles.closeBtn, { backgroundColor: isDark ? 'rgba(255,255,255,0.15)' : 'rgba(0,0,0,0.06)' }]} onPress={handleClose}>
                  <Icon sfSymbol="xmark" mdIcon="close" size={16} color={colors.text} />
                </TouchableOpacity>
              </View>

              {/* Primary Action Glass Card */}
              <TouchableOpacity
                style={[styles.glassCardRow, { backgroundColor: surfaceBg, borderColor: surfaceBorder }]}
                onPress={handleShareNative}
                activeOpacity={0.7}
              >
                <View style={[styles.iconFloat, { backgroundColor: colors.primary }]}>
                  <Icon sfSymbol="square.and.arrow.up" mdIcon="share-variant" size={20} color="#FFF" />
                </View>
                <View style={styles.glassTextContent}>
                  <Text style={[styles.glassTitle, { color: colors.text }]}>{t('Envoyer & Partager')}</Text>
                  <Text style={[styles.glassSubtitle, { color: colors.textSecondary }]}>{t('Ouvrir les options du système')}</Text>
                </View>
                <Icon sfSymbol="chevron.right" mdIcon="chevron-right" size={18} color={colors.textSecondary} />
              </TouchableOpacity>
              ...
              <View style={[styles.glassGroup, { backgroundColor: surfaceBg, borderColor: surfaceBorder }]}>
                ...
                <View style={styles.groupRow}>
                  <View style={[styles.iconFloatSub, { backgroundColor: isDark ? 'rgba(59, 130, 246, 0.15)' : 'rgba(59, 130, 246, 0.1)' }]}>
                    <Icon sfSymbol="globe" mdIcon="earth" size={18} color="#3B82F6" />
                  </View>
                  <View style={styles.glassTextContent}>
                    <Text style={[styles.glassTitle, { color: colors.text, fontSize: 16 }]}>{t('Lien public')}</Text>
                    <Text style={[styles.glassSubtitle, { color: colors.textSecondary }]}>{t('Générer un lien web')}</Text>
                  </View>
                  <TouchableOpacity
                    style={[
                      styles.toggleBtn, 
                ...
                {publicSlug && publicUrl && (
                   <View style={[styles.urlContainer, { backgroundColor: isDark ? 'rgba(0,0,0,0.3)' : 'rgba(0,0,0,0.03)' }]}>
                     ...
                     <TouchableOpacity style={styles.copyBtn} onPress={handleShareNative}>
                       <Icon sfSymbol="doc.on.doc" mdIcon="content-copy" size={16} color={colors.primary} />
                     </TouchableOpacity>
                   </View>
                )}
                ...
                <View style={styles.groupRow}>
                  <View style={[styles.iconFloatSub, { backgroundColor: isDark ? 'rgba(168, 85, 247, 0.15)' : 'rgba(168, 85, 247, 0.1)' }]}>
                    <Icon sfSymbol="person.2.fill" mdIcon="account-group" size={16} color="#A855F7" />
                  </View>
                  <View style={styles.glassTextContent}>
                    <Text style={[styles.glassTitle, { color: colors.text, fontSize: 16 }]}>{t('Collaborer en ligne')}</Text>
                    <Text style={[styles.glassSubtitle, { color: colors.textSecondary }]}>{t('Bientôt disponible')}</Text>
                  </View>
                  <View style={[styles.badgePro, { backgroundColor: isDark ? 'rgba(168, 85, 247, 0.2)' : 'rgba(168, 85, 247, 0.1)' }]}>
                    <Text style={[styles.badgeProText, { color: '#A855F7' }]}>{t('Pro')}</Text>
                  </View>
                </View>
              </View>
              ...
              >
                <View style={[styles.iconFloat, { backgroundColor: '#EF4444' }]}>
                  <Icon sfSymbol="trash" mdIcon="trash-can-outline" size={20} color="#FFF" />
                </View>
                <View style={styles.glassTextContent}>
                  <Text style={[styles.glassTitle, { color: '#EF4444' }]}>{t('Supprimer la note')}</Text>
                  <Text style={[styles.glassSubtitle, { color: isDark ? 'rgba(239, 68, 68, 0.7)' : 'rgba(239, 68, 68, 0.8)' }]}>{t('Action irréversible')}</Text>
                </View>
              </TouchableOpacity>
...
fs.writeFileSync('Mobile/src/components/ShareModal.tsx', content);
