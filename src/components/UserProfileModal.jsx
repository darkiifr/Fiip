...
function ProfileAvatar({ user, profile, onUpload }) {
  ...
  return (
    <div className="relative h-20 w-20 shrink-0 overflow-hidden rounded-3xl border border-warm-border-dark bg-amber-500/10 dark:border-white/10">
      {avatar ? (
...
export default function UserProfileModal({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/45 p-4 font-sans backdrop-blur-md">
      <div className="relative flex max-h-[90vh] w-full max-w-3xl overflow-hidden rounded-[28px] border border-warm-border-dark bg-warm-bg-dark text-warm-text-primary-dark shadow-[0_28px_90px_rgba(0,0,0,0.28)] dark:border-white/10 dark:bg-[#1c1c1b] dark:text-warm-text-primary-dark">
        <aside className="w-48 border-r border-warm-border-dark bg-warm-sidebar-dark/70 p-4 dark:border-white/10 dark:bg-white/[0.035]">
          <p className="mb-3 px-2 text-[10px] font-black uppercase tracking-[0.16em] text-warm-text-muted-dark">Compte</p>
            ...
            {tabs.map(({ id, label, icon: Icon }) => (
              <button
                ...
                className={`flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-xs font-bold transition-colors ${
                  activeTab === id
                    ? 'bg-white text-warm-text-primary-dark shadow-sm dark:bg-white/10 dark:text-white'
                    : 'text-warm-text-secondary-dark hover:bg-white/60 dark:text-warm-text-secondary-dark dark:hover:bg-white/5'
                }`}
              >
                <Icon size={14} />
                {label}
          ...
          <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
            {activeTab === 'profile' && (
              <div className="space-y-6">
                <section className="flex gap-4 rounded-3xl border border-warm-border-dark bg-warm-card-dark p-4 dark:border-white/10 dark:bg-white/[0.045]">
                  ...
                  <div className="min-w-0 flex-1 space-y-3">
                    <label className="block text-[10px] font-black uppercase tracking-[0.14em] text-warm-text-muted-dark" htmlFor="profile-nickname">
                      Nom affiché
                    </label>
                    <input
                      id="profile-nickname"
                      ...
                      className="w-full rounded-2xl border border-warm-border-dark bg-white px-3 py-2 text-sm font-semibold outline-none focus:border-amber-500/45 dark:border-white/10 dark:bg-zinc-950/60 dark:text-white"
                      placeholder={displayName}
                    />
                    <p className="text-xs text-warm-text-muted-dark dark:text-warm-text-muted-dark">L’email reste privé. Fiip affiche ce nom et votre photo.</p>
                  </div>
                </section>

                <section className="space-y-2">
                  <label className="block text-[10px] font-black uppercase tracking-[0.14em] text-warm-text-muted-dark" htmlFor="profile-bio">
                    Bio courte
                  </label>
                  <textarea
                    id="profile-bio"
                    ...
                    className="h-24 w-full resize-none rounded-3xl border border-warm-border-dark bg-warm-card-dark p-4 text-sm outline-none focus:border-amber-500/45 dark:border-white/10 dark:bg-white/[0.045] dark:text-white"
                ...
                <section className="space-y-3">
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-[0.14em] text-warm-text-muted-dark">Compétences</p>
                    <p className="mt-1 text-xs text-warm-text-muted-dark dark:text-warm-text-muted-dark">Choisissez les technologies affichées sur votre profil. Les icônes non prises en charge sont filtrées.</p>
                  </div>
                        ...
                        <button
                          ...
                          className={`flex items-center gap-2 rounded-2xl border px-3 py-2 text-left text-xs font-bold transition-all ${
                            selected
                              ...
                              : 'border-warm-border-dark bg-warm-card-dark hover:bg-warm-sidebar-item-active dark:border-white/10 dark:bg-white/[0.045] dark:hover:bg-white/[0.08]'
                          }`}
                        >
              ...
              <div className="space-y-4">
                ...
                <section className="grid gap-3 sm:grid-cols-2">
                  ...
                  <div className="rounded-3xl border border-warm-border-dark bg-warm-card-dark p-4 dark:border-white/10 dark:bg-white/[0.045]">
                    ...
                    <p className="mt-1 text-xs text-warm-text-muted-dark dark:text-warm-text-muted-dark">{subscription}</p>
                  </div>
                </section>
              </div>
            )}
            ...
            {activeTab === 'legal' && (
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="rounded-3xl border border-warm-border-dark bg-warm-card-dark p-5 dark:border-white/10 dark:bg-white/[0.045]">
                  ...
                  <p className="mt-2 text-sm leading-6 text-warm-text-secondary-dark dark:text-warm-text-secondary-dark">La licence est liée au compte et sert à activer les fonctions Premium sans exposer de clé IA personnelle.</p>
                </div>
              </div>
            )}
          </div>

          <footer className="flex items-center justify-between border-t border-warm-border-dark px-6 py-4 dark:border-white/10">
            <p className={`text-xs font-semibold ${status.includes('pas') || status.includes("n'a") ? 'text-red-500' : 'text-warm-text-muted-dark dark:text-warm-text-muted-dark'}`}>
              {status || 'Les changements sont enregistrés quand vous cliquez sur Enregistrer.'}
            </p>
            <div className="flex gap-2">
              <button type="button" onClick={onClose} className="rounded-2xl border border-warm-border-dark px-4 py-2 text-xs font-bold hover:bg-warm-sidebar-item-active dark:border-white/10 dark:hover:bg-white/10">
                Fermer
              </button>
